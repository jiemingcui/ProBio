...
#os.environ["WANDB_API_KEY"] = "b6343f6cb587568094e820818cd0b03d5fa0e722"
...
def main():
    ...
    #wandb.init(project="AutoBio", name='{}_{}_{}_{}'.format(args.log_time,config['network']['type'], config['network']['arch'], config['data']['dataset']))
    ...
    model, clip_state_dict = clip.load(config.network.arch,device=device,
                                 jit=False, 
    ...
    #model_text = model_text.cuda() #torch.nn.DataParallel(model_text).cuda()
    #model_image = model_image.cuda() #torch.nn.DataParallel(model_image).cuda()
    #fusion_model = fusion_model.cuda() #torch.nn.DataParallel(fusion_model).cuda()

    #wandb.watch(model)
    #wandb.watch(fusion_model)

    train_data = Action_DATASETS(config.data.train_list,
    ...
    val_data = Action_DATASETS(config.data.val_list,
                   config.data.label_list, 
                   random_shift=False,
                   num_segments=config.data.num_segments,
                   image_tmpl=config.data.image_tmpl,
    ...
    for epoch_idx in range(start_epoch, config.solver.epochs):
        model_image.train()
        model_text.train()
        fusion_model.train()

        for batch_idx, (images, class_id, _) in enumerate(tqdm(train_loader)):
            ...
            text_id = numpy.random.randint(num_text_aug, size=len(class_id))
            texts = torch.stack([text_dict[j][i,:] for i, j in zip(class_id, text_id)])
            ...
            ground_truth = torch.tensor(gen_label(class_id), dtype=image_embedding.dtype, device=device)
        ...
        if epoch_idx % config.logging.eval_freq == 0:  # and epoch>0
            prec1 = validate(epoch_idx, val_loader, classes, device, model, fusion_model, config, num_text_aug)

        is_best = prec1 > best_prec1
        best_prec1 = max(prec1, best_prec1)
        filename = "{}/last_model.pt".format(working_dir)
        epoch_saving(epoch_idx, model, fusion_model, optimizer, filename)
        if is_best:
            best_saving(working_dir, epoch_idx, model, fusion_model, optimizer)
